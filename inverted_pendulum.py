...
from tracker import findCenter
...
class InvertedPendulumGame(object):
    ...
    def save_current_state_as_image(self, path):
        pygame.image.save(self.surface, path + "Screenshot_{}.png".format(int(self.pendulum.score)))
    ...
    def game_round(self):
        ...
        initial_state = np.array([0, 0])  # Initial state [theta, theta_dot]
        kf = KalmanFilter(initial_state, np.diag([process_variance, process_variance]), np.array([[observation_variance]]) )
        ...
        for i in range(self.max_timestep):
            ...
            pygame.display.update()
            self.save_current_state_as_image('./screenshots/')
            x_meas, theta_meas = findCenter(int(self.pendulum.score))

            # Initialize Kalman Filter

            if i == 0:
                kf.state[0] = 0  # Initialize theta using the measurement
                theta_meas = 0
            kf.predict()
            kf.update(np.array([theta_meas]))  # Update the Kalman Filter with the observed theta

            estimated_theta = kf.state[0]
            est_t.append(estimated_theta)
            estimated_theta_dot = kf.state[1]
            est_td.append(estimated_theta_dot)

            meas_t.append(theta_meas)

            state = (self.pendulum.terminal, self.pendulum.timestep, self.pendulum.x, self.pendulum.x_dot, estimated_theta, estimated_theta_dot, self.pendulum.reward)
            action = self.PID_controller.get_action(state, self.surface_array, random_controller=self.random_controller)
            ...
            # if self.noisy_actions and PID_controller is None:
            ...
            theta_dot_real.append(np.abs(theta_dot))
            self.timestep = timestep
            self.surface.fill(self.WHITE)
            ...
            if terminal or self.timestep == self.max_timestep:
                plt.plot(np.arange(len(theta_diff_list)), theta_diff_list)
                plt.plot(np.arange(len(meas_t)), meas_t)
                plt.plot(np.arange(len(est_t )), est_t)
                plt.xlabel('Time')
                plt.ylabel('Theta(radians)')
                plt.title("Theta vs Time")
                plt.legend(['Actual theta','Detected theta','Corrected theta'])
                plt.grid()
                plt.savefig("./graphs/" + self.performance_figure_path + "_run_" + str(len(self.score_list)) + ".png")
                plt.close()

                plt.plot(np.arange(len(theta_dot_real)), theta_dot_real)
                plt.plot(np.arange(len(est_td)), est_td)
                plt.xlabel('Time')
                plt.ylabel('Theta dot(radians/s)')
                plt.title("Theta dot vs Time")
                plt.legend(['Actual theta dot','Corrected theta dot'])
                plt.grid()
                plt.savefig("./graphs/speed" + self.performance_figure_path + "_run_" + str(len(self.score_list)) + ".png")
                plt.close()

                # plt.plot(np.arange(len(variance_theta)), variance_theta)
                # plt.plot(np.arange(len(variance_theta_dot)), variance_theta_dot)
                # plt.xlabel('Time')
                # plt.ylabel('variance')
                # plt.title("P variance vs Time")
                # plt.legend(['theta variance','theta dot variance'])
                # plt.grid()
                # plt.savefig("./graphs/variance" + self.performance_figure_path + "_run_" + str(len(self.score_list)) + ".png")
                # plt.close()
                break;
...
def main():
    ...
    if args.mode == "manual":
        ...
        inv = InvertedPendulumGame(args.performance_figure_path, mode=PID_controller.PID_controller(), gravity=args.gravity, manual_action_magnitude=args.manual_action_magnitude,
